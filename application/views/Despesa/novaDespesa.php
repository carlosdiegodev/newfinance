        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h1 id="titulo" class="page-header"><i class="fa fa-external-link-square fa-fw text-danger"></i>Nova Despesa</h1>
                        </div>                        
                    </div>   
                    <?php echo form_open('Despesa/novaDespesa', array('class' => '', 'id' => 'formularioNovaDespesa')); ?>
                    <div class="row">
                        <div class="col-md-4">
                            <?php
                            //Centro de Custo
                        ...
                        <div class="col-md-4">
                            ...
                            foreach ($formasdepagamento as $linha) {
                                $opcoesFormaPagamento[$linha->id_formadepagamento] = $linha->codigo . '-' . $linha->nome . '(Id:' . $linha->id_formadepagamento . ')';
                            }
...
//Campo Lugar da Despesa---------------------------------------------------------------------------------------------------------
                            echo '<div class="form-group">';
                            echo form_label('Local da Despesa<span class="text-danger"> *</span>', '', array('class' => 'control-label', 'id' => ''));
                            echo form_input(array('tabindex' => '9', 'value' => 'Palmas-TO', 'id' => 'localDespesa', 'name' => 'localDespesa', 'class' => 'form-control', 'placeholder' => 'Lojas americanas em Palmas...'), set_value('localDespesa'));
                            echo '</div>';
                            ?>
                        </div>
                    </div>
...
//Botão Nova Finalidade---------------------------------------------------------------------------------------------------------
        ...
        <script type="text/javascript">
                    ...
                    for (var x in dados) {
                        ...
                        if (selectado != null) {
                            if (dados[x].id == selectado) {
                                opcoes += "<option selected='selected' value='" + dados[x].id + "'>" + dados[x].nomeFinalidade + " (Id: " + dados[x].id + ")(" + dados[x].nomeGrupo + ")</option>";
                            } else {
                                opcoes += "<option value='" + dados[x].id + "'>" + dados[x].nomeFinalidade + " (Id: " + dados[x].id + ")(" + dados[x].nomeGrupo + ")</option>";
                            }
                        } else {
                            opcoes += "<option value='" + dados[x].id + "'>" + dados[x].nomeFinalidade + " (Id: " + dados[x].id + ")(" + dados[x].nomeGrupo + ")</option>";
                        }
            ...
            //Essa função abaixo seta o grupo de finalidade de acordo com o id_grupo quando o usuário seleciona a finalidade diretamente.
            //Função mesmo pra facilitar a leitura e entendimento do que o usuário está fazendo.
            function setarGrupoDeFinalidade(id_finalidade) {
                $.ajax({
                    url: base_url + 'Finalidade/listarFinalidadePorId/' + id_finalidade,
                    type: "GET",
                    dataType: "json"
                }).done(function (dados) {
                    for (var x in dados) {
                        //Preencho o grupo de finalidades...
                        $("#id_grupodefinalidade").val(dados[x].id_grupodefinalidade);
                    }

                });
            }
            $(document).ready(function () {
                //Formata o campo multi select da tela de cadastrar nova finalidade
                //$('#novafinalidade_id_centrodecusto').multiselect();
                ...
                $('#id_finalidade').change(function () {
                    ...
                    setarGrupoDeFinalidade(id_finalidade);
                    ...
                    if ($("#id_finalidade").val() != '') {
                        $.ajax({
                            ...
                            for (var x in dados) {
                                ...
                                $("#localDespesa").val(dados[x].localdadespesa);
                                $("#localDespesa").val(dados[x].localdadespesa);
                ...
                //Ação responsável por capturar a ação de select do centro de custo
                $('#dataFato').change(function () {
                    //alert($('#dataReferencia').val());
                    if ($('#dataReferencia').val() == '') {
                        $('#dataReferencia').val($('#dataFato').val());
                        $('#dataPagamento').focus();
                    }
//                    if($('#dataPagamento').val()==''){
//                        $('#dataPagamento').val($('#dataFato').val());                    
//                        $('#valor').focus();
//                    }
                });
                ...
                $('#novafinalidade_id_grupo_existente').change(function () {

                    if ($('#novafinalidade_id_grupo_existente').val() != '' ) {
                ...
                $('#btnNovaFinalidade').click(function () {
                    //preciso setar o centro de custo                        
                    ...
                    if ($("#id_centrodecusto").val() == '') {
                        ...
                        $('#novafinalidade_id_centrodecusto').val($('#id_centrodecusto').val());
                        ...
                        if ($("#id_grupodefinalidade").val() != '') {
                            $('#novafinalidade_id_grupo_existente').val($("#id_grupodefinalidade").val());
                            $("#novafinalidade_nomedogrupo").prop('disabled', true);
                        }
                ...
                $('#novafinalidade_btnsalvar').click(function () {
                    //verificar se vai cadastrar novo grupo
                    if ($('#novafinalidade_id_grupo_existente').val() == '' && $('#novafinalidade_nomedogrupo').val() == '') {
                        alert('Por gentileza, digite o nome do grupo!');
                        return;
                    }
                    ...
                    $.ajax({
                        ...
                        data: {id_centrodecusto: $("#novafinalidade_id_centrodecusto").val(), id_grupodefinalidade: $("#novafinalidade_id_grupo_existente").val(), nomedonovogrupo: $("#novafinalidade_nomedogrupo").val(), nomedanovafinalidade: $("#novafinalidade_nomedafinalidade").val()},
                        type: "post",
                        dataType: "json"
                    }).done(function (dados) {
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="exampleModalLabel">Fechamento de Caixa</h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            $opcoes = array();
                            foreach ($centrodecusto as $linha) {
                                $opcoes[$linha->id_centrodecusto] = $linha->nome . ' (' . $linha->id_centrodecusto . ')';
                            }
                            echo form_multiselect('novafinalidade_id_centrodecusto[]', $opcoes, '', array('class' => 'form-control', 'display' => 'block', 'id' => 'novafinalidade_id_centrodecusto'));
                            ?>
                            <br/>
                            <label for="novafinalidade_id_grupo_existente" class="control-label">Grupo de Finalidades</label>
                            <?php
                            $opcoes = array();
                            $opcoes[''] = 'Cadastrar novo grupo';
                            foreach ($gruposdefinalidades as $linha) {
                                $opcoes[$linha->id_grupodefinalidade] = $linha->nome . ' (' . $linha->id_grupodefinalidade . ')';
                            }
                            echo form_dropdown('novafinalidade_id_grupo_existente', $opcoes, '', array('class' => 'form-control', 'id' => 'novafinalidade_id_grupo_existente'));
                            ?>
